...
#-----------Do 2d convolution using fft----------------------
def fftConv2d(var,kernel,mode='same'):
    '''Do 2d convolution using fft.

    This is an implementation using numpy.fft.
    scipy.signal.fftconvolve() does the same thing but faster.
    '''
    import numpy

    assert numpy.ndim(var)==2, "<var> needs to be 2d."
    assert numpy.ndim(kernel)==2, "<kernel> needs to be 2d."

    #----------------------Pad 0s----------------------
    s1=numpy.array(var.shape)
    s2=numpy.array(kernel.shape)
    assert s2[0]<=s1[0] and s2[1]<=s1[1], "<kernel> size needs to be <= <var>."
    ext_size=s1+s2-1 # make linear, otherwise convolution is circular

    fsize=2**numpy.ceil(numpy.log2(ext_size)).astype('int')
    fslice=(slice(0,ext_size[0]), slice(0,ext_size[1]))

    #-------------------Do fft conv-------------------
    fr=numpy.fft.fft2(var,fsize)
    fr2=numpy.fft.fft2(numpy.flipud(numpy.fliplr(kernel)),fsize)
    cc=numpy.fft.ifft2(fr*fr2)
    cc=cc.real[fslice]

    #--------------Crop shape accordingly--------------
    if mode=='full':
        return cc
    elif mode=='same':
        startidx=(s2-1)//2
        endidx=startidx+s1
        return cc[slice(startidx[0],endidx[0]), slice(startidx[1],endidx[1])]
    elif mode=='valid':
        startidx=s2-1
        endidx=s1
        return cc[slice(startidx[0],endidx[0]), slice(startidx[1],endidx[1])]
...
def conv3D(var,kernel):
    ...
    return fftConv2d(var,kernel,mode='valid')
...
def slideWindowHOG(img,window_size=(128,64),stride=4,
    ...
    for ii in range(orientations):
        ...
        convii=fftConv2d(countii*weights,kernel,mode='valid')
...
if __name__=='__main__':
    ...
    t1=time.time()
    fd2,indices=slideWindowHOG(img,window_size=img.shape[:2],orientations=9,
            pixels_per_cell=(8,8),
    ...
    t1=time.time()
    fd3=slideWindowHOG(img,window_size=img.shape[:2],orientations=9,
            pixels_per_cell=(8,8),
